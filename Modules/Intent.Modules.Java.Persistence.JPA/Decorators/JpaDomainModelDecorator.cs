...
namespace Intent.Modules.Java.Persistence.JPA.Decorators
{
    ...
    public class JpaDomainModelDecorator : DomainModelDecorator
    {
        ...
        public override IEnumerable<string> Fields()
        {
            ...
            if (!HasPrimaryKey(_template.Model))
            {
                ...
                var (type, columnType) = _application.Settings.GetDatabaseSettings().KeyType().AsEnum() switch
                {
                    ...
                    _ => throw new ArgumentOutOfRangeException($@"Database KeyType: {_application.Settings.GetDatabaseSettings().KeyType().AsEnum()} is not supported.")
                };

                var generationType = TryGetGenerationType(_template.Model, out var strategy)
                    ? Environment.NewLine + $@"    @{_template.ImportType($"{JavaxJakarta()}.persistence.GeneratedValue")}(strategy = {strategy})"
                    : string.Empty;
                
                yield return $"""
                              @{_template.ImportType($"{JavaxJakarta()}.persistence.Id")}{generationType}
        ...
        private bool TryGetGenerationType(ClassModel model, out string? strategy)
        {
            var primaryKey = model.Attributes.FirstOrDefault(p => p.HasPrimaryKey());
            return TryGetGenerationType(primaryKey, out strategy);
        }

        private bool TryGetGenerationType(AttributeModel? model, out string? strategy)
        {
            var primaryKey = model?.GetPrimaryKey();
            if (primaryKey?.DataSource()?.Value is null || primaryKey.DataSource().IsDefault())
            {
                strategy = $"{_template.ImportType($"{JavaxJakarta()}.persistence.GenerationType")}.AUTO";
                return true;
            }

            if (primaryKey.DataSource().IsAutoGenerated())
            {
                strategy = $"{_template.ImportType($"{JavaxJakarta()}.persistence.GenerationType")}.IDENTITY";
                return true;
            }
        ...
        public override string FieldAnnotations(AttributeModel model)
        {
                    ...
                    !HasPrimaryKey(_template.Model.ParentClass)))
            {
                ...
                if (TryGetGenerationType(model, out var strategy))
                {
                    annotations.Add($"@{_template.ImportType($"{JavaxJakarta()}.persistence.GeneratedValue")}(strategy = {strategy})");
                }
            }
