...
#line hidden
  ...
  foreach(var attribute in Model.Attributes.Where(x => !x.Name.Equals("id", StringComparison.InvariantCultureIgnoreCase))) { 
    ...
    foreach (var field in GetDecorators().Select(s => s.GetAdditionalFields()).Where(p => !string.IsNullOrEmpty(p)))
    {
  ...
  foreach(var operation in Model.Operations) { 
    ...
    foreach (var method in GetDecorators().Select(s => s.GetAdditionalMethods()).Where(p => !string.IsNullOrEmpty(p)))
    {
